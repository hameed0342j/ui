/**
 * @fileoverview Editable BOM (Bill of Materials) input table.
 */
...
export function BomTable() {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-lg font-semibold text-foreground">BOM Input</h2>
                <p className="text-sm text-muted-foreground mt-1">
                    Enter planned quantities and rates for each order
                </p>
            </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="hover:bg-transparent">
                                ...
                                <TableHead className="w-[120px] text-xs font-semibold uppercase tracking-wider text-right">Planned Qty</TableHead>
                        ...
                        <TableBody>
                            {order.items.map((item) => {
                                const amount = item.plannedQty * item.plannedRate;
                                return (
                                    <TableRow key={item.id} className="h-[44px] transition-colors duration-100">
                                        ...
                                        <TableCell className="text-right p-1">
                                            <Input
                                                ...
                                                onChange={(e) => handleChange(order.id, item.id, 'plannedQty', e.target.value)}
                                        ...
                                        <TableCell className="text-right p-1">
                                            <Input
                                                type="number"
                                                value={item.plannedRate}
                                                onChange={(e) => handleChange(order.id, item.id, 'plannedRate', e.target.value)}
                                                className="w-full text-right text-sm h-8 focus:ring-2 focus:ring-primary/30"
                                            />
                                        </TableCell>
                    ...
                    <div className="px-4 py-2.5 border-t border-border bg-muted/20 flex justify-end">
                        <span className="text-sm font-semibold tabular-nums">
                            Total: {formatCurrency(
                                order.items.reduce((sum, item) => sum + item.plannedQty * item.plannedRate, 0)
                            )}
                        </span>
                    </div>
                </div>
